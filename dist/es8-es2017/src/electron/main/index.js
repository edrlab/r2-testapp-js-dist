...
const zip_ex_http_1 = require("r2-utils-js/dist/es8-es2017/src/_utils/zip/zip-ex-http");
...
async function isManifestJSON(urlOrPath) {
    let p = urlOrPath;
...
file drag-and-drop
    ...
    resetMenu(undefined);
    async function dialogResult(_event, payload) {
...
electron_1.ipcMain.on(events_1.R2_EVENT_OPEN_URL_OR_PATH, async (_event, payload) => {
    await loadFileOrUrl(payload.urlOrPath);
});
async function loadFileOrUrl(argPath) {
    ...
    if (filePathToLoadOnLaunch) {
        if (epub_1.isEPUBlication(filePathToLoadOnLaunch) || await isManifestJSON(filePathToLoadOnLaunch)) {
